...
describe('Builder', () => {
  ...
  it('throws on Uint8ClampedArray with an actionable message', () => {
    const builder = new Builder();
    expect(() =>
      builder.addSlab(new Uint8ClampedArray([1, 2, 3]) as any),
...
describe('top-level encode rejects Uint8ClampedArray', () => {
  it('throws an actionable error', () => {
    expect(() => encode({ x: new Uint8ClampedArray([1, 2, 3]) })).toThrow(
      /Uint8ClampedArray/,
    );
  });
});
